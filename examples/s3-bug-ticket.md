# Scenario 3: Bug Ticket (Regression Test) -- Generating an Inline Regression Test for Checkout Discount Code Bug

> **SparQ Version:** 1.0.0
>
> This example demonstrates Scenario 3 with a bug ticket input. Bug ticket BUG-42 describes a checkout price calculation issue where discount codes are applied twice when a user edits the cart quantity after applying a code. The regression test is appended inline to the relevant feature spec.
>
...
```
User: /sparq:generate-e2e BUG-42
```

## 2. Orchestrator Classification

Classified as Scenario 3 (E2E Test Generation, bug ticket sub-mode). Input matches Jira bug ticket ID pattern (`BUG-42`). Existing e2e/: 8 page objects, 6 components, 3 steps classes, 1 fixture. Dispatching automation-engineer with bug ticket context.
...
- **Relevant feature spec:** `e2e/specs/checkout.spec.ts` -- regression test will be appended here
...
Output: Inline append to e2e/specs/checkout.spec.ts (1 test, REG-BUG42-001 in title)
```
...
### `e2e/specs/checkout.spec.ts` (appended inline)

The regression test is appended as a new `test.describe` block at the end of the existing checkout spec:

```typescript
// --- Regression: BUG-42 ---
test.describe('REG-BUG42-001: Discount double-application on quantity edit', () => {
  let cart: CartPage

  test('REG-BUG42-001: discount is applied only once after editing cart quantity', async ({
    page,
    authSteps,
  }) => {
...
```
[sparq] P2 Regression test appended inline: e2e/specs/checkout.spec.ts -- 3 assertions
```

## 6. Checkpoint 2: Code Review

**Staged:** 2 files (1 inline regression test appended, 1 page object extension) | **Assertions:** 3 (discount badge, discounted total, post-edit total) | **Reused:** CartPage, auth fixture | **Extended:** CartPage with `discountCodeInput`, `applyDiscountButton`, `discountBadge`, `applyDiscount()`, `editItemQuantity()`
...
```
[sparq] Run: npx playwright test --list e2e/specs/checkout.spec.ts
Listing tests:
  [chromium] > checkout.spec.ts > REG-BUG42-001: Discount double-application on quantity edit > REG-BUG42-001: discount is applied only once after editing cart quantity

Total: 1 test in 1 file (plus existing checkout tests)
...
[sparq] Regression test for BUG-42 complete.
  Appended to: e2e/specs/checkout.spec.ts (1 test, REG-BUG42-001)
  Page object: e2e/pages/cart.page.ts (3 accessors + 2 methods added)
  Smoke: PASSED

Run regression tests: npx playwright test --grep "REG-"

Would you like to run /sparq:validate to check the broader test suite?
```
...
- `e2e/specs/checkout.spec.ts` -- Regression test appended inline (1 test, `REG-BUG42-001` in title)
...
|------------|-------------------|--------|
| BUG-42 (Discount code applied twice on quantity edit) | REG-BUG42-001 | Repro steps 1-5: apply discount, edit quantity, verify single application |
