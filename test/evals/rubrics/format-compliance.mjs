 ...
 * Checks: TC ID format, REQ ID format. For S6: REG ID format instead.
 * Scenario-aware: skips checks that don't apply to the scenario's output type.
 */

function checkRegIds(content) {
  const passed = content.match(/REG-[A-Z]+-\d+-\d{3}/g)?.length > 0
  return { passed, finding: 'No valid REG IDs found (expected REG-{ticket}-{NNN})' }
}
...
function collectChecks(content, scenario) {
  // S6 regression tests use REG IDs only
  if (scenario === 'S6') return [checkRegIds(content)]
