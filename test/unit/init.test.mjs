...
describe('cmdInit via CLI subprocess', () => {
  ...
  describe('feature-based init', () => {
    let tempDir

    beforeEach(() => {
      tempDir = createTempDir()
      createMockProject(tempDir, {
        name: 'feature-init-test',
        dependencies: { vue: '^3.4.0' },
        withGit: true,
      })
    })
    ...
    it('should install only specified features with --features minimal', async () => {
      ...
      assert.equal(exitCode, 0, 'Init --features=minimal should exit 0')

      // Core agent must be present
      assert.ok(
        existsSync(join(tempDir, '.claude', 'agents', 'sparq-orchestrator.md')),
        'Core orchestrator agent should be installed',
      )

      // Non-core agents must NOT be present
      assert.ok(
        !existsSync(join(tempDir, '.claude', 'agents', 'sparq-requirements-analyst.md')),
        'Requirements analyst should NOT be installed with minimal preset',
      )
      assert.ok(
        !existsSync(join(tempDir, '.claude', 'agents', 'sparq-automation-engineer.md')),
        'Automation engineer should NOT be installed with minimal preset',
      )
    })

    it('should install all features with --features all', async () => {
      const { exitCode } = await runCli(['init', '--non-interactive', '--features=all', tempDir])

      assert.equal(exitCode, 0, 'Init --features=all should exit 0')
      ...
      for (const agent of expectedAgents) {
        assert.ok(
          ...
          `Agent ${agent} should be installed with all preset`,
        )
      }
    })
  })
